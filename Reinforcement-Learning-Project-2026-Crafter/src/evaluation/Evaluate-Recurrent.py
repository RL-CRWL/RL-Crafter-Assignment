...
class CrafterEvaluator:
    ...
    def evaluate(self, model):
        ...
        for episode in range(self.n_episodes):
            ...
            # Initialize recurrent states for RecurrentPPO
            lstm_states = None
            episode_start = np.ones((1,), dtype=bool)
            
            # Initialize achievement tracking
            achievement_vector = np.zeros(22, dtype=np.float32)
            
            while not done:
                # Transform observation to match model's expected format
                # Model expects: Dict with 'image' (3, 64, 64) and 'achievements' (22,)
                # Env provides: array (64, 64, 3)
                
                # Convert image from (H, W, C) to (C, H, W) and add batch dimension
                image = np.transpose(obs, (2, 0, 1))  # (64, 64, 3) -> (3, 64, 64)
                
                # Get achievements from info if available
                if 'achievements' in info:
                    for i, achievement in enumerate(self.achievements):
                        achievement_vector[i] = float(info['achievements'].get(achievement, 0))
                
                # Create dict observation with batch dimension
                obs_input = {
                    'image': image[np.newaxis, :],  # (1, 3, 64, 64)
                    'achievements': achievement_vector[np.newaxis, :]  # (1, 22)
                }
                
                # Get action from model with recurrent states
                ...
                # After first step, episode_start should be False
                episode_start = np.zeros((1,), dtype=bool)
                
                # Extract action from batch format
                if isinstance(action, np.ndarray):
                    action = action.item() if action.shape == () else action[0]
