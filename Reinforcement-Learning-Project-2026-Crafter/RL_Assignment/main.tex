...
To complement frame-stacking, residual blocks were added to the CNN feature extractor. Residual connections help gradients propagate more effectively through the network, improving training stability and feature representation. While they do not directly provide temporal memory, they strengthen the extracted features, which combined with frame-stacking, can further improve the agent's performance.
...
5.9\% decrease from Improvement 1 (197.66 steps) and 3.6\% below baseline (193.02 steps). The agent unlocked 7 out of 22 achievements with a geometric mean of 43.80\%, showing a 23.7\% improvement over Improvement 1's geometric mean (35.42\%) despite unlocking fewer total achievements. The standard deviation of 1.37 remained relatively stable compared to Improvement 1 (1.30).
...
\begin{itemize}
	...
	\item \textbf{Achievement Progression:} Even though Improvement 2 has lower mean reward, it actually does better on achievements, as shown in Figure \ref{fig:dqn_comp_geo_mean}. The geometric mean steadily increases: baseline (24.9\%) to Improvement 1 (37.2\%) and then to Improvement 2 (44.2\%), giving a 49\% improvement over baseline and 19\% improvement over Improvement 1. Looking at the achievement heatmap (Figure \ref{fig:dqn_comp_heatmap}), Improvement 2 focuses on doing fewer achievements really well (84\% unlock rates for collect\_drink, collect\_stone, collect\_wood, and wake\_up) instead of trying to unlock a wider variety of achievements.
	
	\item \textbf{Survival Time:} Figure \ref{fig:dqn_comp_survival} shows consistent survival across all three models, with mean survival times of 191, 191, and 197 steps for baseline, Improvement 1, and Improvement 2 respectively. This means the basic survival abilities stayed roughly the same across all modifications, and the main differences in performance come from how the agents make strategic decisions rather than just staying alive in the environment.
\end{itemize}
\textbf{Conclusion:} The results show an interesting trade-off between the two improvements. Improvement 1 achieves the best immediate reward performance with higher mean rewards and more stable episode outcomes, making it effective for short-term optimization. However, Improvement 2 demonstrates superior long-term strategic planning, as evidenced by its significantly higher geometric mean achievement score (44.2\%) despite lower mean rewards. This suggests that Improvement 2's architectural changes shifted the agent's behavior from reward maximization to goal-oriented planning, focusing on completing specific achievements with high success rates rather than accumulating immediate rewards. The consistent survival times across all models indicate that the core survival capabilities remain intact, and the performance differences reflect different strategic priorities rather than fundamental competence issues. Overall, Improvement 1 is better for immediate reward tasks, while Improvement 2 excels at structured, multi-step goal completion.
